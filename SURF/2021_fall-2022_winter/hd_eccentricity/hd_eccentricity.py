...
# STARTING MULTIPROCESSING FOR eccentricities:
print('STARTING MULTIPROCESSING FOR eccentricities:')
...
Ks_101 = np.logspace(0, 1, nsims)  # from 10^0 = 1 to 10^1 = 10
# e1_track, e2_track = np.zeros(len(Ks)), np.zeros(len(Ks))
par_list_ecc_101 = []
for i, k in enumerate(Ks_101):
    filename_ecc_101 = f'ecc_track_archive_cluster_100_101_{i}.bin'  # each value of k gets its own file
    par_list_ecc_101.append((planet_masses, k, filename_ecc_101))

# parallelize it
pool_ecc = InterruptiblePool()
ecc_track_results = pool_ecc.map(get_eccentricity_tracks, par_list_ecc_101)  # get e1s, e2s
